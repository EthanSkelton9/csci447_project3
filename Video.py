...
class Video():
    ...
    def performance(self):
        ...
        for ((data, NN), v) in prod(zip([rdata, cdata], [rNN, cNN]), vectors):
            print("=====================================================")
            print("Our data is {}".format(str(data)))
            print("With {} hidden layers, we will use the vector {}".format(len(v), v))
            DataCV = CV(data)
            test_fold = DataCV.training_test_dicts(data.df)[1][0]
            y = NN.stochastic_online_gd(test_fold)(eta=0.1, hidden_vector = v, alpha=0.9)
            y_show = NN.prediction(y, data.classes) if data.classification else y
            print("Our predicted values are:")
            print(y_show)
            r = NN.targetvec(data.classification, data.classes)
            error = NN.calc_error(y, r, data)
            print("~~~~~~~~~~~~~~~~~~~~~~~")
    ...
    def grad_calc(self):
        ...
        cNN = Neural_Net(cdata)
        rNN = Neural_Net(rdata)
        for (data, NN) in zip([rdata, cdata], [rNN, cNN]):
            print("=====================================================")
            print("Our data is {}".format(str(data)))
            vec_func = NN.vec()
            r = NN.targetvec(data.classification, data.classes)
            nrows = data.df.shape[1] - 1
            target_length = len(data.classes) if data.classification else 1
            ws_init = pd.Series(NN.list_weights(nrows, [3, 3], target_length))
            (yi, i, grads, new_ws) = NN.online_update_single(vec_func, r, 0.1, 0, data.df.index)(data.df.index, ws_init, None, [])
            print("With starting weights of:")
            print(ws_init)
            print("We get a value of {} at index {}.".format(yi, i))
            print("Our gradients are:")
            print(grads)
            print("Our new weights are:")
            print(new_ws)
